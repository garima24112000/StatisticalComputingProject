...
# Select numeric predictors for VIF check (remove factors and response)
dfVif = df %>% select(where(is.numeric))

# Remove any highly missing columns if present
dfVif = dfVif[, colSums(is.na(dfVif)) == 0]

# Fit a linear model with gdp as the response (you can change this)
vifModel = lm(gdp ~ ., data = dfVif)

# Compute VIF
vifValues = vif(vifModel)
print(vifValues)

# Optional: View high VIFs (>5)
highVif = vifValues[vifValues > 5]
...
# Step 1: Select numeric features for clustering (exclude factor columns like income_level, continent)
dfNumeric = df %>% select(where(is.numeric))

# Step 2: Scale the numeric data
dfScaled = scale(dfNumeric)

# Step 3: Run K-means with your desired k (e.g., 3 for income levels)
set.seed(123)
...
# Plot clusters colored by cluster ID
...
# Step 6: Optional — Evaluate clustering alignment using confusion matrix
# Remap clusters based on majority vote to match income_level
clusterMapping = df %>%
  group_by(cluster) %>%
  count(income_level) %>%
...
library(cluster)
silhouetteScore = silhouette(kmeansResult$cluster, dist(dfScaled))
mean(silhouetteScore[, 3])
